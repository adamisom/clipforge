...
<div className="timeline">
  <div className="time-ruler">{/* Generate tick marks every second */}</div>
...
const VideoPreview = ({ clip, currentTime }) => {
  ...
  return <video ref={videoRef} src={`file://${clip.filePath}`} controls />
}
```
...
const TimelineClip = ({ clip, onTrim }) => {
  return (
    ...
    >
      <div className="clip-content">{clip.name}</div>
    </ResizableBox>
  )
}
```
...
const splitClipAtPlayhead = (clipId: string, playheadTime: number) => {
  ...
  setTimelineClips((clips) => clips.filter((c) => c.id !== clipId).concat([leftClip, rightClip]))
}
```
...
const exportVideo = (resolution: '720p' | '1080p' | 'source') => {
  ...
  ffmpeg().input(/* ... */).videoFilters(filters).output(outputPath)
}
```
...
const Toast = ({ title, message, type }) => (
  <div className={`toast toast-${type}`}>
    <div className="toast-icon">{type === 'error' ? '⚠️' : '✓'}</div>
    <div>
...
const FORBIDDEN_PATHS = ['/System', '/Library', '/Windows/System32']
...
ffmpeg().input(clip1).input(clip2).complexFilter([
  // Fade out clip1
...
const TimelineClip = React.memo(
  ({ clip, onDrag, onResize }) => {
    return <div className="clip">{/* Clip UI */}</div>
  },
  (prevProps, nextProps) => {
...
{
  clips.map((clip) => <Clip key={clip.id} onDrag={(e) => handleDrag(clip.id, e)} />)
}
...
{
  clips.map((clip) => <Clip key={clip.id} clipId={clip.id} onDrag={handleDrag} />)
}
```
...
const ThumbnailImage = ({ clip }) => {
  ...
  return <div ref={imgRef}>{loaded ? <img src={clip.thumbnail} /> : <Skeleton />}</div>
}
```
